...
pipeline {
    ...
    stages {
        ...
        stage('Generate Variables') {
            steps {
                sh "${bob()} publish-generate-varaiables"
            }
        }
        ...
        stage('SonarQube analysis') {
            steps {
                withSonarQubeEnv("${env.SQ_SERVER}") {
                    sh "${bob()} sonar-enterprise-publish"
                }
            }
        }
        ...
        stage('Generate Helm') {
            steps {
                withSonarQubeEnv("${env.SQ_SERVER}") {
                         sh "${bob()} create-helm-build-vars-public"
        ...
        stage('Generate Docker') {
            steps {
                sh "${bob()} create-image-build-name-public"
        ...
        stage('Generate Zip') {
            steps {
                sh "${bob()} publish-zip-public:zip-sample-app"
                sh "${bob()} publish-zip-public:validate-zip"
            }
        }
        ...
        stage('publish') {
            parallel {
                ...
                stage('Docker') {
                    // bob rule 'create-image-build-name-public' needs to run before the publish 'Docker' stage to
                    // populate the 'image-build-name' variable
                    steps {
                        withCredentials([usernamePassword(credentialsId: 'SELI_ARTIFACTORY', usernameVariable: 'SELI_ARTIFACTORY_REPO_USER', passwordVariable: 'SELI_ARTIFACTORY_REPO_PASS')]) {
                            sh "${bob()} build-docker-image:push-docker-image"
                        }
                    }
                }
                stage('Publish Zip') {
                    steps {
                        withCredentials([usernamePassword(credentialsId: 'SELI_ARTIFACTORY', usernameVariable: 'SELI_ARTIFACTORY_REPO_USER', passwordVariable: 'SELI_ARTIFACTORY_REPO_PASS')]) {
                            sh "${bob()} publish-zip-public:zip-upload-public-repo"
                        }
                    }
                }
                stage('Publish Contract Tests') {
                    steps {
                        script{
                            try {
                                sh "${bob()} contract-testing:create-docker-network"
                                sh "${bob()} contract-testing:run-sample-app-container"
                                sh "${bob()} publish-contract-testing"
                            }
                            finally {
